package com.blinked.repositories.base;
...
import com.blinked.annotations.SensitiveConceal;
 ...
 */
@NoRepositoryBean
public interface BaseRateRepository<RATE extends BaseRate> extends BaseRepository<RATE, Long>, JpaSpecificationExecutor<RATE> {
    ...
    @NonNull
    @SensitiveConceal
    List<RATE> findAllByProductIdAndStatusAndParentId(@NonNull Integer productId, @NonNull RateStatus status, @NonNull Long parentId);
    ...
    @NonNull
    @SensitiveConceal
    Page<RATE> findAllByProductIdAndStatusAndParentId(Integer productId, RateStatus status, Long parentId, Pageable pageable);
    ...
    @Query("select new com.blinked.entities.projection.RateChildrenCountProjection(count(rate.id), rate.parentId) " +
            "from BaseRate rate " +
            "where rate.parentId in ?1 " +
            "group by rate.parentId")
    @NonNull
