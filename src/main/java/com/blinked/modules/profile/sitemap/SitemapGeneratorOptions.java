...
public class SitemapGeneratorOptions {
	...
	/**
	 * Split large sitemap into multiple files by specifying sitemap size. Default
	 * 5000.
	 */
	@Builder.Default
	...
	/**
	 * <p>
	 * Array of relative paths (wildcard pattern supported) to exclude from listing
	 * on sitemap .xml or sitemap-*.xml.
	 * </p>
	 *
	 * <p>
	 * e.g.: ['/page-0', '/page-*', '/private/*'].
	 * </p>
	 * Apart from this option next-sitemap also offers a custom transform option
	 * which could be used to exclude urls that match specific patterns
	 */
	...
	public SitemapEntry transform(String url) {
		...
		SitemapEntry.SitemapEntryBuilder builder = SitemapEntry.builder().loc(loc).changefreq(changefreq)
				.priority(priority);
