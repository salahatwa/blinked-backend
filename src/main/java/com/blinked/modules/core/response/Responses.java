package com.blinked.modules.core.response;
...
import com.blinked.modules.core.utils.JSON;
 ...
 * HTTP Responses.
 * <p>
 * Classe util para simplificar a geração de responses para status codes comuns
...
public class Responses {
	...
	public static final <T> ResponseEntity<T> created(T body, String location, String id) {
		return ResponseEntity.created(URI.create(String.format("/%s/%s", location, id))).body(body);
	}
	...
	public static void forbidden(HttpServletResponse response) {
		...
		try {
			...
			response.getWriter()
					.write(JSON.stringify(new ApiError("Can't find bearer token on Authorization header.", FORBIDDEN)));
	...
	public static void expired(HttpServletResponse response) {
		...
		try {
			response.setStatus(FORBIDDEN.value());
			response.setContentType("application/json");
			response.getWriter().write(JSON.stringify(new ApiError("Token expired or invalid.", FORBIDDEN)));
