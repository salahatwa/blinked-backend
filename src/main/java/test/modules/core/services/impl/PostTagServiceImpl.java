...
public class PostTagServiceImpl extends AbstractCrudService<PostTag, Integer> implements PostTagService {
	...
	public PostTagServiceImpl(PostTagRepository postTagRepository, PostRepository postRepository,
			TagRepository tagRepository, OptionService optionService) {
	...
	public List<TagWithPostCountDTO> listTagWithCountDtos(Sort sort) {
		...
		// Find all post count
		Map<Integer, Long> tagPostCountMap = ServiceUtils.convertToMap(postTagRepository.findPostCount(),
				TagPostPostCountProjection::getTagId, TagPostPostCountProjection::getPostCount);
	...
	public Map<Integer, List<Tag>> listTagListMapBy(Collection<Integer> postIds) {
		...
		// Foreach and collect
		postTags.forEach(postTag -> tagListMap.computeIfAbsent(postTag.getPostId(), postId -> new LinkedList<>())
